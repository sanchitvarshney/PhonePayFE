...
import R5ReportDetailTable from "@/table/report/R5ReportDetailsTable";
...
const R5Report: React.FC = () => {
  ...
  const { r5reportLoading, r5ReportDetail, r5ReportDetailLoading, r5report } =
    useAppSelector((state) => state.report);
  const [filter, setFilter] = useState<string>("DEVICE");
  const [device, setDevice] = useState<DeviceType | null>(null);
  const [deviceType, setDeviceType] = useState<string>("soundbox");
  const { emitDownloadR5Report, isConnected } = useSocketContext();
  ...
  const downloadr5report = () => {
    if (filter === "DEVICE") {
      if (!device || !deviceType)
        return showToast("Please select a device", "error");
  ...
  const handleSearch = () => {
    if (filter === "DEVICE") {
      if (!device || !deviceType) {
        showToast("Please select a device and device type", "error");
      } else {
        dispatch(
          getr5Report({
            type: "DEVICE",
      ...
      >
        <div className="border-b h-[50px] border-neutral-300 bg-neutral-200 flex items-center px-[20px]">
          ...
          >
            {r5ReportDetail &&
              `#${
               r5ReportDetail?.productionData?.productSrlNo
              }`}

            {r5ReportDetailLoading && (
                  ...
                  secondary={
                    <>
                      {r5ReportDetailLoading ? (
                        ...
                        r5ReportDetail?.productionData?.productionLocation
                      )}
                    </>
                  }
                />
                  ...
                  secondary={
                    <>
                      {r5ReportDetailLoading ? (
                        ...
                        r5ReportDetail?.productionData?.dropLocation
                      )}
                    </>
                  }
                />
                  ...
                  secondary={
                    <>
                      {r5ReportDetailLoading ? (
                        ...
                        r5ReportDetail?.productionData?.insertDate
                      )}
                    </>
                  }
                />
                  ...
                  secondary={
                    <>
                      {r5ReportDetailLoading ? (
                        ...
                        r5ReportDetail?.productionData?.insertBy
                      )}
                    </>
                  }
                />
          ...
          <div className="flex  gap-[20px] flex-col   p-[20px] overflow-hidden mt-[20px]">
            ...
            <div className="flex justify-between itesms-center">
              <div className="flex gap-[10px]">
                ...
                <LoadingButton
                  disabled={!isConnected || !r5report}
                  onClick={downloadr5report}
                  variant="contained"
