import React from "react";
import { Outlet, useLocation, useNavigate } from "react-router-dom";
import { Box } from "@mui/material";
import Tabs from "@mui/material/Tabs";
import Tab from "@mui/material/Tab";

const PHONEPE_PURPLE = "#5F259F";

const ProductionModuleLayout: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const tabRoutes = ["/production-module/add", "/production-module/report"];
  const currentTabIndex = tabRoutes.indexOf(location.pathname);

  const handleChange = (_: React.SyntheticEvent, newValue: number) => {
    navigate(tabRoutes[newValue]);
  };

  return (
    <div className="h-full">
      <div className="w-full h-[50px] border-b border-neutral-300 bg-white">
        <Tabs
          sx={{
            padding: 0,
            width: "max-content",
            "& .MuiTab-root": { color: "#64748b" },
            "& .Mui-selected": { color: PHONEPE_PURPLE },
            "& .MuiTabs-indicator": { backgroundColor: PHONEPE_PURPLE, height: "3px" },
          }}
          value={currentTabIndex === -1 ? 0 : currentTabIndex}
          onChange={handleChange}
          centered
        >
          <Tab label="Add Production Data" />
          <Tab label="Production Report" />
        </Tabs>
      </div>

      <Box sx={{ height: "calc(100vh - 100px)" }}>
        <Outlet />
      </Box>
    </div>
  );
};

export default ProductionModuleLayout;

