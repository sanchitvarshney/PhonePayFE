...
const POCellRenderer: React.FC<POCellRendererProps> = ({
  props,
  customFunction,
}) => {
  ...
  const { currencyData, currencyLoaidng } = useAppSelector(
    (state) => state.common
  );
  ...
  const handleChange = (val: string) => {
    ...
    if (data.gstType === "L") {
      data["sgst"] =
        ((Number(data.gstRate) / 100) * Number(data.taxableValue)) / 2;
      data["cgst"] =
        ((Number(data.gstRate) / 100) * Number(data.taxableValue)) / 2;
      data["igst"] = 0;
    } else {
      ...
      data["cgst"] = 0;
      data["igst"] =
        (Number(data.gstRate) / 100) * Number(data.taxableValue);
    }
    api.refreshCells({
      rowNodes: [props.node],
      columns: [
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (data.excRate != 0 || data.excRate != "") {
      data["taxableValue"] =
        Number(data.qty) * Number(data.rate) * Number(data.excRate);
    }
    if (data.gstType === "L") {
      data["sgst"] =
        ((Number(data.gstRate) / 100) * Number(data.taxableValue)) / 2;
  ...
  const renderContent = () => {
    switch (colDef.field) {
        ...
        return (
          <AntCompSelect
            getUom={(val) => {
              ...
              api.refreshCells({
                rowNodes: [props.node],
                columns: [column, "component", "remark", "qty", "uom"],
              });
              customFunction();
            }}
            onChange={(selectedValue) => {
              dispatch(
                getPOComponentDetail(selectedValue?.value || "")
              ).then((res: any) => {
            ...
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                <Button type="button" variant="outlined">
                  {currencyData?.find((item: any) => item.id === value)?.text}
                </Button>
              </PopoverTrigger>
              <PopoverContent
                className="flex flex-col gap-[10px] w-[350px] shadow-none"
                style={{
                  boxShadow:
              ...
              >
                ...
                <div className="flex items-center gap-[10px]">
                  <Select
                    ...
                    onChange={(e: string) => {
                      ...
                      if (
                        currencyData?.find((item: any) => item.id === e)?.text ===
                        "₹"
                      ) {
                        data["foreignValue"] = 0;
                        data["taxableValue"] =
                  ...
                  <Input
                    disabled={
                      currencyData?.find(
                        (item: any) => item.id === data.currency
                      )?.text === "₹"
                    }
                    value={currency}
                ...
                <div className="flex items-center justify-end gap-[10px] h-[50px] mt-[10px]">
                  <Button
                    ...
                    startIcon={
                      <IoMdCheckmark className="h-[18px] w-[18px]" />
                    }
                    variant="contained"
                  >
                    Submit
      ...
      case "taxableValue":
        return (
          <span>
            {value % 1 == 0 ? value : value?.toFixed(2) ?? "0.00"}
          </span>
        );
      case "foreignValue":
        return (
